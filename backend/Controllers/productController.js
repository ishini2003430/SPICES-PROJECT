...
exports.updateProduct = async (req, res) => {
  try {
    ...
    // Validate required fields, numeric fields, and packaging sizes only if not Out of Stock
    if (status !== "Out of Stock") {
      ...
      // If Out of Stock, just update status and any provided fields
      ...
      if (req.body.packagingSizes) {
        try {
          const parsedPackagingSizes = JSON.parse(req.body.packagingSizes);
          if (Array.isArray(parsedPackagingSizes)) {
            updateData.packagingSizes = parsedPackagingSizes;
          }
        } catch (e) {}
      }
      if (req.file) {

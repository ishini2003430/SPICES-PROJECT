...
function AddIncome() {
  ...
  const handleSubmit = (e) => {
    ...
    // Validate title and category length (between 3 and 50 characters)
    if (inputs.title.length < 3 || inputs.title.length > 50 ||
        inputs.category.length < 3 || inputs.category.length > 50) {
      setError("Title and Category must be between 3 and 50 characters");
      return;
    }
    ...
    if (!validText.test(inputs.title) || !validText.test(inputs.category)) {
      setError("Title and Category must contain only letters and spaces");
      return;
    }

    // Validate amount format (max 2 decimal places)
    if (!/^\d+(\.\d{0,2})?$/.test(inputs.amount)) {
    ...
    sendRequest().then(() => history("/incomesdisplay"));
  };

  // Send POST request to backend
  const sendRequest = async () => {
    await axios
      .post("http://localhost:5000/incomes/add", {
